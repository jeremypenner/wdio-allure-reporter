...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('allure:attachfile', ({ cid, attachmentName, filename, mimetype, toTest }) => {
            const allure = this.getAllure(cid)
            const test = allure.getCurrentTest()
            if (!attachmentName)
                attachmentName = path.basename(filename)
            const outfilename = path.resolve(options.outputDir, path.basename(filename))
            fs.createReadStream(filename).pipe(fs.createWriteStream(outfilename))
            const attachment = new Attachment(attachmentName, outfilename,
                fs.statSync(filename).size, mimetype || 'application/binary')
            if (!toTest) {
        ...
        this.on('allure:attachdata', ({ cid, attachmentName, data }) => {
            const allure = this.getAllure(cid)
            this.dumpJSON(allure, attachmentName, data)
        })

        this.on('allure:feature', ({ cid, feature }) => {
            const allure = this.getAllure(cid)
            const test = allure.getCurrentTest()
            test.addLabel('feature', feature)
        })

        this.on('allure:startstep', ({ cid, label }) => {
            const allure = this.getAllure(cid)
            allure.startStep(label)
        })

        this.on('allure:endstep', ({ cid, status, label }) => {
